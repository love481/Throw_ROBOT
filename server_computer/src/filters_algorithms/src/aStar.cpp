...
#include<vector>
bool finishedSearched;
std::vector<std::vector<double>> Path;
int AStar::AStarSearch::totalNodeExpanded = 0;
void AStar::AStarSearch::drawRoute(Node* parentHead)
{    //start routing from the node other than start
	std::cout << "x:" << parentHead->y << "\t" << "y:" << Row - parentHead->x << std::endl;
	Path.push_back({( double)parentHead->y ,(double)Row - parentHead->x});
...
void AStar::AStarSearch::startSearch()
{
		input_map[start->x][start->y] = 5;
		input_map[des->x][des->y] = 6;
		...
		{  
			...
			drawRoute(parentHead);
			for (int i = 0; i < INT_MAX; i++);
			finishedSearched = true;
		}
